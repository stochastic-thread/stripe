defmodule Stripe.Response do

  @type t :: %Stripe.Response{
    status_code: number,
    data: term,
    raw_response: Stripe.Response.http_potion_response
  }
  ...
  @spec new(HTTPotion.Response) :: Stripe.Response | {:error, term}
  def new(response) do
    new(response, %{})
  end

  @spec new(HTTPotion.Response, map) :: Stripe.Response | {:error, term}
  def new(response, options) do
    case parse_body(response, options) do
      {:ok, data} ->
        %Stripe.Response{ data: data, raw_response: response, status_code: response.status_code }
      {:error, error} ->
        {:error, error}
    end
  end
  ...
  @spec success?(Stripe.Response.http_potion_response) :: boolean
  ...
  defp cast_data(data, %{as: as_struct}) when is_map(data) do
    ...
    struct(as_struct, Stripe.Utils.safe_atomize_keys(data, struct_keys))
  end
